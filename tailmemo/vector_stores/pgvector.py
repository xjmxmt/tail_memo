...
class PGVector(VectorStoreBase):
    ...
    def search(self, query, vectors, limit=5, filters=None, distance_metric=DistanceMetric.COSINE, distance_threshold=0.1):
        """
        ...
        Args:
            ...
            distance_threshold (float, optional): Similarity distance threshold (0-1). Only return results with distance <= distance_threshold.
        ...
        # 添加距离阈值过滤：只返回 distance <= distance_threshold 的结果
        filter_conditions.append(f"vector {distance_op} %s::vector <= %s")
        filter_params.extend([vectors, distance_threshold])
